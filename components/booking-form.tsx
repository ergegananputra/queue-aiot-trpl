...
export function BookingForm({ computers, selectedComputerId }: BookingFormProps) {
  ...
  const now = new Date();
  const defaultStartDate = formatJakarta(now, "yyyy-MM-dd");
  const defaultStartTime = formatJakarta(now, "HH:mm");
              ...
              <div className="flex flex-wrap gap-2">
                {occupiedComputers.map((c) => (
                  <span key={c.id} className="px-2 py-1 text-xs rounded-full bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200">
                    {c.name} - Free at {formatJakarta(c.nextAvailableAt!, "MMM d, HH:mm")}
                  </span>
                ))}
              </div>
            )}
                        ...
                        {computer.isOccupied ? (
                          <span className="text-xs text-muted-foreground">
                            (Free at {computer.nextAvailableAt
                              ? formatJakarta(computer.nextAvailableAt, "MMM d, HH:mm")
                              : "unknown"})
                          </span>
                        ) : (
